...
public:
    ...
    template <typename... Ts>
        requires((std::is_base_of_v<IComponent, Ts> && ...))
    auto entities_with() {
        ...
        auto filtered =
            base_entities | std::views::filter([this](EntityID e) { return (get_pool<Ts>().has_component(e) && ...); });

        // Concatenate tuple to return EntityID, Component&...
        return filtered | std::views::transform([this](EntityID e) {
                   return std::tuple_cat(std::make_tuple(e), std::forward_as_tuple(get_pool<Ts>().get_component(e)...));
               });
    }
    ...
    struct ComponentPool : public IComponentPool {
        ...
        T& get_component(EntityID entity_id) {
            if (!has_component(entity_id)) {
                throw std::runtime_error("An entity doesnt have the required component!");
            }
        ...
        void remove_component(EntityID entity_id) {
            // Does nothing if element doesn't exist
