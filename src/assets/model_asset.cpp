...
ModelAsset::ModelAsset(std::string name) : m_name(name.empty() ? "unnamed_model" : std::move(name)) {
}

ModelAsset::ModelAsset(std::string name, AssetID mesh_id) : ModelAsset(std::move(name)) {
    m_meshes.push_back(mesh_id);
}

std::shared_ptr<ModelAsset> ModelAsset::create_fallback() {
    auto model = std::make_shared<ModelAsset>("fallback_model");
    model->add_mesh(AssetManager::instance().get_fallback_id<MeshAsset>());
    return model;
}

std::optional<std::shared_ptr<ModelAsset>> ModelAsset::load_from_file(const std::string& path,
    ...
    if (!scene || scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || !scene->mRootNode) {
        ERR("[ModelAsset] Assimp load error: " << importer.GetErrorString())
        return std::nullopt;
    }

    auto model = std::make_shared<ModelAsset>(std::string(name));
    ...
    for (uint32_t m = 0; m < scene->mNumMeshes; m++) {
        ...
        // Create mesh
        AssetID mesh_id =
            am.add_asset<MeshAsset>(std::move(ai_mesh->mName.C_Str()), VertexFormat::POS_NOR_TEX, vertices.data(),
                                    vertices.size(), indices.data(), indices.size(), mat_id);
...
const std::string& ModelAsset::name() const {
    return m_name;
}
