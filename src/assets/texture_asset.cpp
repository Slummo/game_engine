...
std::optional<std::shared_ptr<TextureAsset>> TextureAsset::load_from_file(const std::string& path, TextureType type,
    ...
    if (!res) {
        ERR("[TextureAsset] Failed to upload texture: " << path);
        return std::nullopt;
    }
...
GLuint TextureAsset::get_id() const {
    return m_id;
}
...
TextureType TextureAsset::type() const {
    return m_type;
}
