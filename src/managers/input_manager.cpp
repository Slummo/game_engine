...
void InputManager::on_scroll(double /*xoffset*/, double yoffset) {
...
void InputManager::on_action_pressed(const std::string& name, ActionCallback cb) {
    m_action_pressed_callbacks[name].push_back(std::move(cb));
}

void InputManager::begin_frame(double current_time) {
    m_time = current_time;
...
bool InputManager::is_mouse_button_down(MouseButton b) const {
    if (b < 0 || b >= 32) {
        return false;
    }
...
glm::dvec2 InputManager::mouse_pos() const {
    return m_mouse_pos;
}

glm::dvec2 InputManager::mouse_delta() const {
...
void InputManager::set_mouse_pos(glm::dvec2 pos) {
    m_mouse_pos = pos;
}

void InputManager::set_mouse_delta(glm::dvec2 delta) {
    m_mouse_delta = delta;
}

void InputManager::register_action(const Action& a) {
    m_actions.insert_or_assign(a.name, a);
}

void InputManager::register_action(const std::string& name, std::vector<Binding> bindings) {
    Action action(name);
    for (auto& b : bindings) {
...
float InputManager::get_axis(const std::string& axis_name) const {
...
bool InputManager::get_action_down(const std::string& name) const {
...
bool InputManager::get_action_pressed(const std::string& name) const {
...
void InputManager::emit_event(const Event& ev) {
    m_events.push_back(ev);
}

float InputManager::eval_binding(const Binding& b) const {
    switch (b.type) {
        case InputType::Key: {
